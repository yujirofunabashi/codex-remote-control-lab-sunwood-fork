...
Install the official native binary independently on each target PC. Run `agy` in that PC's terminal and complete Google sign-in there; do not copy credentials between machines or put authorization codes in chat. Then run:
...
The first integration is intentionally limited. The composer displays a plan-mode notice and disables attachments and the ordinary access-mode switch. `--mode plan` asks the agent to propose a plan; **it is not a filesystem read-only boundary**. Local Antigravity permission rules still apply. There is no remote approval protocol: a tool that needs confirmation can be soft-denied even if the final text response succeeds. The bridge reports that distinction and never enables `--dangerously-skip-permissions`. Global `always-proceed`, `useG1Credits: true`, and an alternate `modelProvider` block this account-only path; settings are not rewritten. API key environment variables are cleared only in the child process. Verify local settings before using this experimental path with valuable files.
