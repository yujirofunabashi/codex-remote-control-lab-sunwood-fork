...
The normal public `site.webmanifest` is token-free and uses `display: standalone`. A valid protected `/install?token=...` page links to a no-store install manifest whose `start_url` carries the credential in a URL fragment. This is necessary because Safari and the installed Home Screen web app have separate storage. On first launch, the app saves the credential in its own local storage and immediately removes the fragment; fragments are never sent to the bridge. Invalid or tokenless install-manifest requests cannot receive a credential-bearing `start_url`.
