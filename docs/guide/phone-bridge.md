...
Generated guest configuration deliberately sets `aiExecutionVerified: false`: live file browsing does not authorize prompt delivery, native execution or model-network attachment. The UI retains drafts and labels AI work as pending. Before enabling execution, integrate the lab's currently validated sandbox policy and verify denial of tool access to credentials and modification of previous evidence on hardware. If a temporary sandbox exception is needed, obtain owner approval for its scope, duration and removal; a prior one-task approval does not authorize this recurring workflow. Changing the boolean is neither approval nor verification.
...
The printed command verifies the entire package hash before running `lab_install_host.py`. The installer verifies the stopped/disconnected VM and the exact previous medium, creates only a new `phone-bridge-*` directory beside the existing guard, restricts that new directory to Administrators/SYSTEM with readback, and starts the host relay manually. It does not overwrite old files, swap media, boot the VM or launch AI. Keep the Windows console open after `Relay connected`; manually restart the installed `lab_host.py --config <host.json>` after a Windows reboot. When the owner chooses to keep the lab connected, register that same command once, from an administrator PowerShell, as a scheduled task that starts at Windows startup under the administrator account (S4U, highest privileges) and restarts after a failure, and run the relay under a login LaunchAgent on the relay Mac. This keeps only the connection up: the VM still starts and stops on explicit requests from the phone, and no AI request is scheduled. On the Home Screen the lab card then offers the Windows Codex icon, since the relay serves its own `/install` page. Host connectivity is not proof of guest browsing or application deployment.
