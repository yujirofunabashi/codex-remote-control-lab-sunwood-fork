...
`already has an active writer`（別の実行元が会話を使用中）と表示された場合は、別に起動したCodexやパソコン側のアプリが同じ会話を開いています。そちらの作業を保存して会話を閉じてから、スマホの「同じ会話に再接続」を押します。使用権がまだ解放されていなければ案内が再表示されます。中継ソフトが相手の作業を強制終了することはありません。選択したMac・AI・会話・フォルダ・下書きを保持し、保存済み履歴を読みながら復旧を待てます。依頼文を自動で再送したり、この競合を理由に別のAIへ切り替えたりしません。今後ターミナルから使う場合は、後述の共有接続を使うと競合を避けられます。
