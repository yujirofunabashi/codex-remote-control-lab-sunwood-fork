...
以前の会話の保存履歴を確認できない場合は、失敗画面の「履歴を探す」から一覧を開けます。「同じフォルダで新しく開く」は、元の会話と下書きを残したまま、同じMac・AI・フォルダで履歴のない会話を始める操作です。元の履歴を復元する操作ではありません。フォルダが分からない場合は、フォルダ選択画面から指定します。会話が開くまでは、後から届く待機状態でエラー表示を消しません。
