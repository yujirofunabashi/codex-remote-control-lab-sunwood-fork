...
ホームフォルダ（`~`）そのものに紐づく session は一覧に出しません。素の shell から `claude` を起動したときや、それを内部で呼ぶツール（メモリ系の hook、要約処理など。最初のメッセージが system prompt になっているもの）がここに溜まるためです。出したい場合は `PHONE_CLAUDE_LIST_HOME_SESSIONS=1` を設定してください。bridge 自身がホームで動いている場合は、この除外は適用されません。
