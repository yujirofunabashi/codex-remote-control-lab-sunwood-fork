...
### Windows内の隔離された実験室（接続準備・実機未検証） {#windows-lab}
...
3. 実験室が停止・通信切断済みで、別の実験が動いていないことを確認する。既存のWindows/実験室の防御処理、起動媒体、実行ソフトの版を照合する。`scripts/build_lab_seed.py`（=通信なしの導入媒体の材料を作る処理）へ、確認済みの実験室設定と通信設定を渡す。出力は `user-data`・`meta-data`・`network-config`（=導入内容・実験室の識別・通信設定の3ファイル）で、媒体へのまとめ方は既存の実験室の導入手順に従う。既存ディスクや本人ログインを作り直す処理ではない。
...
実験室内では、通常ユーザーのCodex CLI（=文字の指示でAIを動かすソフト）を使い、AIによる書き込みは作業フォルダと自身の認証・会話記録用フォルダに限定します。実験室の管理処理と信頼側の記録には書き込めません。AIの道具による通信は許可せず、AI提供元への通信は既存の防御処理を維持します。OS（=プログラムの動作を管理する基本ソフト）による1回600秒の制限と、Windows側の660秒の通信切断処理を設けています。実験室の管理待機も30分で終了します。自動のAI依頼や機器起動時のWindows接続処理は登録しません。

現在の検証範囲は `npm run test:lab`（=限定操作・切断・再送の検査）と `npm run smoke:lab`（=2種類の実ブラウザを使う画面検査）です。Windowsの相手役は検査用の代替処理であり、Hyper-V（=Windowsの仮想PC機能）の実機接続、実際のAIによる変更・検査、通常停止は別途確認が必要です。画面検査の成功を実機への導入完了や、公開・支出の承認として扱わないでください。
