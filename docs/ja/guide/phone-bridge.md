...
このアプリのCodex依存版は0.156.1に固定しています。`npm run version:codex`（=アプリ内に導入したCodexの版を確認する命令）は、このアプリの実体だけを使い、見つからない場合に端末全体の別の版へ切り替えません。`CODEX_BIN`（=起動するCodexを別途指定する設定）を使う場合と、すでに動いているCodexの版は、導入版とは別に確認します。
...
このアプリ内の会話から再起動を依頼した場合、再起動でその返答を作る処理も停止します。承認された更新を一度限りの処理へ渡すときは、対象の保存版を固定し、その返答の保存と全会話の待機を確認してから実行します。復旧後は起動時刻、稼働中の版、配信ファイル、履歴・作業場所・接続設定の保持を照合します。待機中の登録は反映完了ではありません。下書き・添付がある画面を強制更新せず、手元の画面表示を直接確認していない場合は、その点も未確認として報告します。
...
表示された起動命令は、受信した内容の照合後に `scripts/lab_install_host.py`（=Windows側の初回準備）を実行します。停止・通信切断・元の媒体を再確認し、新しい `phone-bridge-*`（=今回専用の置き場）だけを作成して接続係を手動起動します。既存ファイルの上書き、媒体の入れ替え、実験室やAIの起動は行いません。接続キーを含むファイルの閲覧権限は、その新しい置き場だけで管理者とWindows本体に限定します。画面に `Relay connected`（=中継との接続成功）と出たら、その文字画面は開いたままにします。2026-09-25からは、オーナーが登録した予定タスク `PhoneBridgeWindowsLab`（=Windows起動時の自動実行）が、保存された `lab_host.py` と `host.json`（=接続処理とその非公開設定）で接続係だけを起動します。中継側の Mac では、ログイン時に動く LaunchAgent（=Macの自動起動の登録）で中継を常駐させます。中継は自分の `/install` ページを持つため、接続先一覧の Windows実験室のカードから、中継が使えるAIごとの Windows アイコンをホーム画面に追加できます。仮想PCやAIは起動しません。Default Switch（=Windows標準の仮想ネットワーク）は再起動のたびに番地が変わるため、Windows側の防御処理（v9）は番地を `192.168.240.1/20` に固定した専用スイッチ `agent-lab-net` と、その番地変換を検査します。最初の接続成功と、専用フォルダの実機確認や既存画面への反映は別の確認段階です。

初回導入で生成する設定は `aiExecutionVerified: false`（=この接続からのAI実行は無効）です。フォルダの閲覧ができても、指示の送信、AIの起動、そのための通信接続は認めません。画面は「AI作業は準備中」と表示し、入力の下書きは残します。過去の実験で1回だけ承認された特別な実行許可は、この接続で繰り返し使える許可ではありません。この値を変えるだけで承認や検証を済ませたことにはなりません。

2026-09-25にオーナーが、作業フォルダ内だけ・外部送信と支出なし・1回600秒までの範囲で、この実験室のAI実行（CodexとClaude）を承認しました。導入済みの実験室は `build_lab_seed.py` の `build_upgrade`（=更新用媒体の作成）で更新します。この媒体は処理ファイルだけを差し替え、実験室内の既存設定（利用者・作業場所・Codexの場所とモデル・防御処理）を `lab_guest_upgrade.py`（=実験室内で1回だけ動く引継ぎ処理）で引き継ぎます。Claudeは、Windows監督が導入・照合したリリース（`claude-2.1.267`）と専用のログイン場所（`/home/agent-lab/.claude-lab`）がそろう場合だけ追加し、そろわなければCodexだけで動きます。固定番地の専用スイッチには番地の自動配布がないため、この媒体は実験室の番地を `192.168.240.2/20`（出口 `192.168.240.1`）に固定します。承認があっても、実験室は起動のたびにAIを使わない閉じ込め確認を、AIごとに実際の実行と同じ制限で行います。作業フォルダだけに書き込めること、制御ファイル・状態記録・もう一方のAIのログイン情報を読めないこと、直接の外部通信ができないことを確かめ、合格したAIだけを起動します。

AI実行処理は、通常ユーザーのCodex CLIまたはClaude Code（=文字の指示でAIを動かすソフト）を使い、OS（=プログラムの動作を管理する基本ソフト）による1回600秒の制限と、Windows側の660秒の通信切断処理を設けています。各AIが書き込めるのは作業フォルダと自分のログイン場所だけで、もう一方のログイン場所と実験室の状態記録は見えません。Codexは道具による通信を許可せず、Claudeは設定・フック・外部接続機能を無効にした制限モードで、作業フォルダ内の編集と命令実行だけを使います。AI提供元への通信は既存の防御処理と、提供元ごとの宛先限定の中継（Codexは既存、Claudeは実行中だけ起動）に限ります。1つの会話は最初に選んだAIを使い続け、AIを替えるには新規セッションを作ります。中継側で `claudeModel` と `claudeEffort` を設定した場合だけ、画面にClaudeを表示します。実験室の管理待機も30分で終了します。管理処理の起動前に、実際に入っている媒体と起動時の識別記録を照合し、別の実験の媒体なら接続も停止処理も始めません。自動のAI依頼は登録しません。機器起動時に動くのは接続係だけです。
...
### Windows本体のCodexを1フォルダに固定して使う（Windowsでは未実行） {#windows-host}

上の実験室とは別の接続です。通常の bridge を Windows 本体で動かし、その PC の Codex を使います。この PC には証券ソフトがあり、bridge にはスマホ用の鍵1本で届きます。そのため、鍵を持っていても Codex の制限を越えられないようにする必要があります。`PHONE_LOCKDOWN_ROOT` に専用フォルダを1つ指定すると、bridge は次のように動きます。

- スマホがどのモードを送っても、Codex の会話と実行はすべて `workspace-write`（=そのフォルダの中だけ書ける）と `on-request`（=フォルダ外の操作は許可を求める）で動かします。スマホの権限ボタンは「確認モード（固定）」と表示され、切り替えられません。誰も答えない承認は待ち続け、そのコマンドは実行されません。
- 作業フォルダとフォルダの閲覧は、その中だけです。`PHONE_WORKSPACE_ROOTS` は無視し、会話の一覧にもその中の会話だけを出します。PC の Codex アプリも、同じ場所に履歴を持っているためです。
...
各PCで[公式の導入手順](https://antigravity.google/docs/cli/install/)を確認し、そのPCのターミナル（文字で操作するアプリ）で、空の試験用フォルダへ移ってから `agy` を起動してGoogleへログインします。ホーム全体を試験用の作業場所として信頼登録しないでください。認証コードは本人が同じターミナルへ入力し、チャットへ貼ったり別PCへコピーしたりしません。初期設定では、会話データを製品改善に使う選択と、作業フォルダへの信頼確認を別々に判断します。その後、同じ試験用フォルダで次を確認します。
...
Macでは、ログインした利用者だけでなく、中継ソフトが実際に使う起動環境も合わせて確認します。Antigravity CLI 1.2.2では、SSH（遠隔の命令接続）を検知するとファイル内の認証情報を使う動作を確認しました。一方、PCの画面からのログインは[OSの保管庫を使います](https://antigravity.google/docs/cli/install/#local-silent-keyring-sign-in)。この違いにより、画面ではログイン済みでも、SSHからの `agy models` だけがログインを要求する場合があります。

その場合は再ログインを繰り返さず、PC本体または実際の中継ソフトと同じ起動環境で、上のモデル一覧と実回答を確認します。SSHの環境情報を外すだけでは、保管庫へのアクセス権は得られません。認証情報のコピー、保管庫の許可範囲の拡大、追加課金への切替で解決しないでください。実回答に成功しても、稼働中のリモコンへの反映完了とは区別します。

確認後、リモコンのAI切替や新規セッション（新しい会話）の画面でGeminiを選びます。リモコンで使う試験用フォルダは、そのPCの利用者用フォルダ配下に置きます。外側の一時フォルダを使うために、既存の作業場所の制限を緩めないでください。未対応の古い中継ソフトとWindows実験室では、使える選択肢として表示しません。Windowsにソフトを入れることと、実験室からAIを動かせることは別です。稼働中のリモコンへの反映には、既存の更新手順と実機確認も必要です。
...
画面には「文章相談（試験）」と表示します。毎回、文章だけで提案・推敲を行い、ファイル操作や命令実行をしないように依頼します。これは依頼の方針であり、**厳密な読み取り専用ではありません**。操作命令の展開を止める `--disable-slash-commands` と組み合わせると、Antigravityの `--mode plan` は無効になるため、この試験版はそのモードを指定・表示しません。実際の操作の許可はPC側の設定に従います。[公式の画面なし接続方式](https://antigravity.google/docs/cli/headless/)には承認ボタン用の通信がないため、確認が必要な処理は実行されない場合があります。文章の回答が成功しても、その処理まで成功したとは表示しません。全操作の自動承認、追加クレジットの使用、別の課金接続が設定されている場合は、設定を勝手に直さず起動を止めます。
...
## モデルと考える深さの選択 {#model-selection}

必要な精度を保ち、仕事に適したモデル（=回答や判断を担うAIの種類）とエフォート（=考える深さ）を選びます。Astra（=GPT-6の高能力モデル）やFable（=Claudeの高能力モデル）への一律固定はしません。複雑な判断には能力と深さを確保し、十分な精度を直接確かめられる小さな仕事では軽めの設定も使います。

[公式のモデル選択資料](https://learn.chatgpt.com/docs/models#choosing-astra-sol-terra-and-luna)を踏まえ、難しい一連の仕事はAstra、複雑で曖昧な仕事はSol、範囲が明確な日常作業はTerraが候補です。Lunaは入出力が明確で反復可能、全結果を検査可能、速い応答が必要、失敗の影響が小さく戻せる、のすべてを満たす場合に限ります。例えば、決めた分類への振り分けを全件検査する作業です。この限定条件は本運用の品質方針であり、公式の能力上限の説明ではありません。

通常の実装・調査は `xhigh`（=非常に深く考える）を基本にし、狭い確定作業は `high`（=深く考える）、必要以上に重いと確認できる単純な作業は `medium`（=中程度）や `low`（=軽め）、最も難しい判断は `max`（=最大の深さ）を使い分けます。選択は仕事の区切りで行い、結果が不足したら見直します。`Ultra`（=複数AIの並行作業）は本人が求めた場合だけです。

アプリはGPT-5.6以上という最低世代を、画面の候補、設定保存、送信受付と送信直前で検査します。古いモデルの会話と保存値は読み出せますが、次の送信には適格なモデルの明示選択が必要です。拒否した入力は画面に残し、別モデルへ自動で送信しません。接続先が返したモデル一覧を取得済みなら、その中の適格な候補だけを使います。候補が空なら停止します。一覧を一度も取得していない場合の初期候補は利用可能の確認ではなく、実際に使えるかは接続先でも検査されます。

Codexで対応する深さは選択したモデルの一覧に従い、指定した対応済みの値をそのまま送ります。古い画面などから未対応の深さが届いた場合も送信を止めます。モデルと深さの選択は独立で、最高モデル・最大深さへ自動で引き上げません。Claudeの既存候補も維持します。この処理は最低世代と対応状況の検査であり、依頼の難しさやLunaの利用条件を自動判定するものではありません。無人実行の候補登録や起動時の共通設定も変更しません。

`node scripts/model-policy-smoke.js` と `node scripts/model-policy-smoke.js --webkit`（=2種類のブラウザで送信と選択を確かめる命令）は、実画面と模擬接続を使い、旧選択の拒否、下書き・添付の保持、軽めの深さの送信を検査します。外部AIへ依頼は送りません。
...
案件を外付けディスクに置く Mac では、`.env` の `PHONE_WORKSPACE_ROOTS` に書いたフォルダだけをホーム外でも開けます。絶対パスを `:` で区切って並べ、空白を含むときは引用符で囲みます（例 `PHONE_WORKSPACE_ROOTS="/Volumes/JIRO SSD 1TB/20_mini"`）。何も書かなければホーム外は開きません。許可したフォルダはホームの一覧の先頭に「外付け: …」として出て、そこから「↑ 上の階層」を押すとホームに戻ります。同じディスクのほかの場所や、許可したフォルダの中から外を指すシンボリックリンク（＝別の場所への近道）は開けません。`/`、`/Volumes` のような最上位のフォルダ、ホームを含む・ホームの中の場所は無視します。ディスクが外れているときは一覧に出ず、その場所を指定した会話は別の場所で始まらずに失敗します。値を変えたら bridge を再起動します。外付けディスクの場合、macOS はその Mac の画面に「リムーバブルボリューム上のファイルへのアクセス」の確認を 1 回だけ出します。誰かが答えるまでフォルダの読み込みが待たされ、bridge 全体が応答しなくなります。phone から初めて使う前に、その Mac の前か画面共有で確認に答えてください。バックアップや保管用ではなく、そのディスクの決まりでその Mac の作業用とされている場所を選んでください。
...
保存先の Mac に貼った場合はその場で起動します。もう一方に貼った場合は SSH（別のパソコンへ安全に接続する仕組み）を使います。前提として、mini 側の接続名 `air` が Air の会話所有者へ、Air 側の接続名 `mini` が mini の会話所有者へ接続できる設定が必要です。それ以外の機種は実機名を接続先として使います。2 台目の mini（実機名 `minijiro2`）もこれに当たり、Air と mini の両方に、その会話所有者へつながる接続名 `minijiro2` が必要です。Mac mini の既定の実機名はどれも「mini」と読まれるため、2 台目の bridge は `PHONE_MACHINE_LABEL=mini2` で起動します。これで会話一覧・色・ホーム画面アイコンが 1 台目の mini と混ざりません。接続後は実機名をもう一度照合し、一致しなければ中止します。接続失敗時に別の Mac で会話を始めることはありません。
...
ヘッダー（=画面上部）の接続先の隣に、小さな「操作元 ?」を表示します。押して「Airで直接」「miniで直接」「mini2で直接」「Airからminiの画面」「Airからmini2の画面」などを選ぶと、「操作 Air」「操作 Air·共有」のような名札になります。選択はこのブラウザ内だけに保存され、会話やAI、接続先のMacを切り替えても手元の設定を勝手に変更しません。実行先は接続先のMacが自分の情報を添えます。
...
対象はこのアプリから送るCodex・Claudeへの依頼です。Codexは `additionalContext`（=会話本文と別に渡すアプリの補足情報）に対応するapp-server（=Codexとの通信を受け持つプログラム）が必要で、開発時は0.153.4の生成仕様を確認しています。Claudeは、変わらない扱いの規則だけを `--append-system-prompt`（=通常の指示に補足を足す起動指定）に渡し、その回の操作元・受信時刻を通常入力の別の文章部分に添えます。生の保存記録には当回データを残し、アプリの履歴と会話一覧では本人の本文だけを表示します。`/clear`（=会話の初期化）などスラッシュで始まる命令には当回データを足さず、元の命令をそのまま渡します。次の通常送信で再び最新の情報を添えます。

[Claudeの再開仕様](https://code.claude.com/docs/en/prompt-caching#resuming-a-session)では、既存会話は開始時の固定指示を保持します。新しい当回情報を固定指示だけに入れると更新が届かない場合があるため、この2つを分けています。順番待ちが長引いた場合も、送信時に選択の期限を再確認し、古ければ手元を未確認に戻します。公式アプリやターミナルから直接送った発言、Windows実験室への依頼には、この仕組みから現在の操作元を付与しません。元の保存記録は書き換えません。

送信内容の機械試験と、実際のAIが操作元を正しく理解する試験、入力の再利用・消費量の測定、本番反映は別の確認です。固定文が同じという試験だけで削減率や回答品質は証明しません。既存会話を消去・強制再読込して検証を代替せず、本番への反映・再起動は更新手順に従います。

## PWA 注意

通常公開する `site.webmanifest` は token を含まず、`display: standalone` を使います。有効な protected `/install?token=...` page だけが no-store の install manifest を参照し、その `start_url` は credential を URL fragment で渡します。以前の protected access により Safari に credential が残っていれば、短い `/install` URL を開くだけで、ホーム画面へ追加する前に protected install page へ自動再読込します。Safari とホーム画面 Web App は storage が別であるため、この受け渡しが必要です。初回起動時に Web App 自身の local storage へ保存し、fragment は即時削除します。fragment は bridge へ送信されず、無効または token なしの install-manifest request が credential 付き `start_url` を受け取ることもありません。`/install?token=...&provider=codex` のように `provider` を付けると、install page・アイコン・manifest はその provider の画像と名前（例: `Codex mini`）になり、`start_url` にも `provider` が残るため、そのアイコンはその provider で開きます。Claude 既定の bridge でも Codex 用アイコンを作れます。ホーム画面のアプリ自体には追加の案内が出ないため、接続先一覧の各カードに「Codexアイコン」「Claudeアイコン」ボタンがあります（その Mac の接続キーが保存されていて、その AI が使える場合だけ）。押すと、その Mac の `/install` ページが保存済みの接続キー付きで開きます。共有 →「ホーム画面に追加」を選んでください。アプリ内の閲覧画面で開いてその項目が無いときは、先に Safari で開き直します。アイコンは、背景の色と大きな名前で機械（Air は青、mini は琥珀色、mini2 は紫、Windows は緑）を、中央の図柄と下のラベルで AI（Codex は黒いターミナル、Claude は白い吹き出し）を表します。`python3 scripts/generate_bridge_icons.py` で一式を描き直せます。iOS は追加した時点の絵を保持するため、新しい絵にするには古いホーム画面アイコンを削除して追加し直します。
