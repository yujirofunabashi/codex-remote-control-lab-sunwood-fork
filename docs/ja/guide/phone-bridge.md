...
Codex（AI作業ツール）の 0.153.4 では、最初のメッセージを送る前の会話を開き直すと、`list_turns is not supported yet`（会話を読む処理の未対応エラー）が出る場合があります。中継ソフトは元の保存ファイルを確認し、会話番号が一致する基本情報だけで本文がない場合に限り、保存された元のフォルダで新しい入力画面を開きます。元のファイルは残し、画面にも理由を表示します。本文がある会話、処理中の会話、読み取れない会話は自動で置き換えません。
...
単独の `codex` 命令で既に開いている会話は、その作業を終えて閉じてから共有の実行元で再開します。今後パソコンから先に始める場合は、作業フォルダ内で `codex --remote <接続先>`（既存の実行元につなぐ起動命令）を使います。Claude は従来の `claude --resume <会話ID>` と記録の監視を使います。Codex の共有接続は Claude の実行方式を変更しません。
...
`npm run smoke:handoff`（引き継ぎの自動確認命令）で、実際の Codex と両ブラウザの動作を確認できます。`codex` と `tmux`（独立した端末画面を用意するソフト）が実行できる環境が必要です。検証専用の保存場所と端末を作り、Mac 内の模擬応答を使うため、普段の認証情報や外部 AI への送信は使いません。
