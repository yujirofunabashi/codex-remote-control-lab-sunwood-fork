...
// One Home Screen icon per AI the lab offers, each opening the lab in that AI.
const labIcons = {
  codex: { name: "Codex Windows実験室", shortName: "Codex Windows", icon180: "bridge-icons/codex-windows-180.png", icon512: "bridge-icons/codex-windows-512.png" },
  claude: { name: "Claude Windows実験室", shortName: "Claude Windows", icon180: "bridge-icons/claude-windows-180.png", icon512: "bridge-icons/claude-windows-512.png" },
};
...
// With a key the page links the key-carrying manifest; without one it links
// none, so Safari cannot make an icon that opens without a key.
function labInstallPage(html, shell, provider = "codex", phoneToken = "") {
  const icon = labIcons[provider];
  const manifest = phoneToken ? `site.webmanifest?provider=${provider}&install=1&token=${encodeURIComponent(phoneToken)}` : "";
  let page = html
    .replace(/<title>[^<]*<\/title>/, `<title>${icon.name}</title>`)
    .replace(/<link rel="apple-touch-icon" href="apple-touch-icon\.png" \/>/, `<link rel="apple-touch-icon" sizes="180x180" href="${icon.icon180}" />`)
    .replace(/<meta name="apple-mobile-web-app-title" content="[^"]*" \/>/, `<meta name="apple-mobile-web-app-title" content="${icon.shortName}" />`)
...
// Each AI gets its own app id, so the Codex and Claude icons are two Home
// Screen apps rather than one replacing the other.
function labManifest(base, provider = "codex", phoneToken = "") {
  const icon = labIcons[provider];
  return {
    ...base,
    name: icon.name,
    short_name: icon.shortName,
    id: provider === "codex" ? "/codex-remote-windows-lab" : `/codex-remote-windows-lab-${provider}`,
    scope: "/",
    description: `Windows lab relay for the phone bridge (${provider}).`,
    icons: [
      { src: `/${icon.icon180}`, sizes: "180x180", type: "image/png", purpose: "any" },
      { src: `/${icon.icon512}`, sizes: "512x512", type: "image/png", purpose: "any maskable" },
    ],
    start_url: phoneToken ? `/install?provider=${provider}#token=${encodeURIComponent(phoneToken)}` : `/?provider=${provider}`,
  };
}
    ...
    try {
      ...
      if (url.pathname.startsWith("/api/")) {
        ...
        if (req.method === "GET") {
          ...
          if (url.pathname === "/api/status") return reply(200, { provider: provider || "codex", ...workspace(config.workRoot), lab: store.target(), projectProgress: readLabProgress(config.progressFile), bridges: Object.values(store.state.threads).map(thread => ({ threadId: thread.id, provider: providerOf(thread), model: models[providerOf(thread)].model, ...workspace(thread.cwd), run: store.run(thread) })) });
      ...
      // The icon's AI must be one this relay offers; anything else gets Codex.
      const installProvider = providerNames.includes(url.searchParams.get("provider")) ? url.searchParams.get("provider") : "codex";
      if (url.pathname === "/install") {
        const authenticated = sameSecret(url.searchParams.get("token"), config.phoneToken);
        res.writeHead(200, { "content-type": "text/html; charset=utf-8", "cache-control": "no-store", "x-content-type-options": "nosniff" });
        return res.end(labInstallPage(fs.readFileSync(path.join(publicDir, "index.html"), "utf8"), shell, installProvider, authenticated ? config.phoneToken : ""));
      }
      if (url.pathname === "/site.webmanifest") {
        ...
        const manifest = labManifest(JSON.parse(fs.readFileSync(path.join(publicDir, "site.webmanifest"), "utf8")), installProvider, install ? config.phoneToken : "");
