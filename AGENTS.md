...
- For edits from multiple machines, use a dedicated feature branch/worktree and share reviewed commits through `origin/develop`. On the receiving application checkout, use `npm run bridge:check` and `npm run bridge:pull`; never silently stash, reset, force-push, or copy over another machine's unfinished work. Resolve refused updates by inspecting and integrating both changes on a feature branch.
- Application delivery is not established by the selected chat folder's branch or by matching HEAD alone. Verify both bridges' authenticated `/api/bridge/info` build identities, clean/shared state, actual served UI, and `restartRequired: false`. Restart only the affected bridge with authorization after active work is saved; preserve each machine's credentials, registry and histories locally. Run `npm run check:sync` and `npm run smoke:fleet` when changing this update path.
