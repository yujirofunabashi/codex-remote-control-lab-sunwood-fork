...
To run multiple phone bridges in parallel, start them on different `PHONE_UI_PORT` values and usually pin each port to a different `PHONE_WORKDIR` worktree. The port is the stable workspace slot; the browser UI can switch between Codex and Claude within that slot. When `CODEX_APP_SERVER_PORT` is not set, each slot uses `PHONE_UI_PORT - 1` for its local Codex app-server, such as `45224 -> 45223`, so slots do not fight over the `45214 -> 45213` default. Settings saved from the browser UI are stored as port-scoped `.env` keys such as `PHONE_WORKDIR_45224`, so changing one slot does not rewrite every port's default worktree. `PHONE_AGENT_PROVIDER` only chooses the default provider opened after restart. The Codex model list follows the app-server's own `model/list`: the bridge asks for it whenever the app-server starts or the phone lists models, keeps the last answer in `.phone-codex-models.json`, and offers it in the composer menu and the settings sheet ahead of a built-in fallback, so a model that reaches the account is selectable without updating this repository. To get a Home Screen icon that opens in the other provider, add `provider=codex` (or `provider=claude`) to the protected `/install?token=...` URL: the page, its icon and its manifest then use that provider's picture and name, such as `Codex mini`, and the icon starts in that provider. The `追加用` install link in the UI adds the provider of the chat that is open. Each bridge has its own active queue and PWA identity, so multiple URLs can be added to the iPhone Home Screen as separate icons. Set `PHONE_APP_NAME`, `PHONE_APP_SHORT_NAME`, and `PHONE_APP_ID` when you want a slot label such as `Slot 45224`. If iOS already cached an older icon, delete that Home Screen icon and add the URL again.

Bridge Fleet / Worktree Switchboard lets one browser tab register several bridge URLs, switch the active worktree, and monitor running/error/approval state across inactive bridges. Open one bridge as usual, tap the current bridge/worktree pill, and paste protected startup URLs or base URLs plus tokens. Tokens are masked in the UI; saved host profiles keep base URLs and metadata separate from the local device token store, and session-only tokens stay in `sessionStorage`. Each bridge also exposes token-protected `GET /api/bridge/info` metadata for label, port, cwd, branch, dirty summary, model, and fleet capabilities. A session stays on the Mac that ran it, so the sidebar collects the thread list from every registered bridge into one list. Project headings and thread rows name the machine (`mini`, `Air`) only while more than one is on screen, each in that machine's own colour to match its Home Screen icon, and opening another Mac's thread moves the connection to that bridge. The workspace and folder controls in settings name the machine they are browsing, with chips at the top for switching which Mac is being configured. The machine name comes from the hostname unless `PHONE_MACHINE_LABEL` overrides it.
...
Gemini is available as an experimental text-conversation provider through **Antigravity CLI (`agy`)**, Google's successor to Gemini CLI. Install and sign in independently on each machine using the [official installation instructions](https://antigravity.google/docs/cli/install/), then verify the available models with `agy models` before choosing Gemini in the app. `ANTIGRAVITY_BIN` selects the executable; `GEMINI_MODEL` (or its port-scoped form) selects an exact Gemini model, defaulting to `gemini-3.8-flash-high`. `PHONE_AGENT_PROVIDER=gemini` also selects it at startup. This uses account authentication, not API keys or automatic credit fallback. The initial integration supports text, saved phone conversations, explicit native-conversation resume and interruption. It sends an advisory-only instruction with every turn, **not a native plan mode or read-only sandbox**, and does not support image/file attachments, interactive tool approvals, terminal handoff, or completion notifications. See [Gemini setup and limits](docs/guide/phone-bridge.md#gemini) before using it on a real project. A browser smoke test is not proof of a successful account login or model response.
...
Notifications for Codex and Claude are opt-in. `PHONE_NTFY_TOPIC` sends them to an ntfy topic, `PHONE_PUSHOVER_TOKEN` plus `PHONE_PUSHOVER_USER` sends them through Pushover, and `PHONE_DISCORD_WEBHOOK_URL` posts them to Discord. Startup sends one message with the Mac's name and the connection URL. Task completion/interruption notifications are always sent through configured providers. Set `PHONE_NOTIFY_EVENTS=1` to also send work events such as `approval_required`, `approval_expired`, `question_required`, `test_failed`, `connection_lost`, `history_sync_failed`, and `long_running`; `PHONE_NOTIFY_EVENT_DEDUPE_MS` controls short-window dedupe for non-forced events. Messages are written in Japanese for a person to read: the first line says which Mac's AI did what (for example `✅ mini の Claude の作業が終わりました`; the Mac's name is `PHONE_MACHINE_LABEL`, or the hostname), then the folder, the start of the request, the start of the answer on completion (200 characters by default; `PHONE_NOTIFY_EXCERPT_CHARS` changes it, `0` leaves it out), the command or file an approval is for, the question when there is one, the cause of a failure, and one link to open. On Discord the post is also made under a per-Mac name such as `Claude mini` or `Codex Air`, with the body in an embed coloured for that Mac (mini amber, mini2 violet, Air blue; `PHONE_BRIDGE_COLOR` overrides it), so the sender is visible at a glance. Thread IDs, turn IDs, model names, and raw event types are not included. A turn that ends on a question sends the question only, not a completion notice as well. The link uses the HTTPS address `tailscale serve` publishes for that bridge when there is one, and the tailnet/LAN IP otherwise. Startup notifications can include the tokenized ready URL for compatibility, so use a private/protected topic, account, or channel. Event notifications use token-free bridge URLs.
...
- one thread sidebar across every registered bridge, with each project heading naming the Mac it is on
